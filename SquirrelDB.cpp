...
static void handleCreate(const cxxopts::ParseResult& result) {
	if (result.count("name")) {
		...
		SquirrelDB::Factory::KVStore::createEmptyDatabase< std::string, std::string>(dbName);
	}
	else {
...
static void handleDestroy(const cxxopts::ParseResult& result) {
	if (result.count("name")) {
		...
		SquirrelDB::Factory::KVStore::createEmptyDatabase< std::string, std::string>(dbName).completelyDestroyDatabase();
	}
	else {
		std::cerr << "Error: Please specify a database name using --name [name]." << std::endl;
	}
...
static void handleSetKeyValue(const cxxopts::ParseResult& result) {
	...
	// load the db
	SquirrelDB::KeyValueStore db = SquirrelDB::Factory::KVStore::loadExistingDatabase<std::string, std::string>(result["name"].as<std::string>());
